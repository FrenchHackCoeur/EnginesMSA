...
class Preferences:
    ...
    def is_item_among_top_10_percent(self, item: Item, item_list: List[Item]) -> bool:
        ...
        for item_ in item_list:
            scores.append((item_, item_.get_score(self)))

        # On trie les scores
        scores = sorted(scores, key=lambda x: x[1], reverse=True)

        # On détermine l'index de l'item
        idx_item = [idx for idx, obj in enumerate(scores) if obj[0] == item][0]

        # Calcul de la position pour les 10 %
...
if __name__ == '__main__':
    ...
    hydrogen_engine = Item("Hydrogen Engine", "A very environment friendly engine")
    agent_pref.add_criterion_value(CriterionValue(hydrogen_engine, CriterionName.PRODUCTION_COST,
                                                  Value.BAD))
    agent_pref.add_criterion_value(CriterionValue(hydrogen_engine, CriterionName.CONSUMPTION,
                                                  Value.VERY_BAD))
    agent_pref.add_criterion_value(CriterionValue(hydrogen_engine, CriterionName.DURABILITY,
                                                  Value.GOOD))
    agent_pref.add_criterion_value(CriterionValue(hydrogen_engine, CriterionName.ENVIRONMENT_IMPACT,
                                                  Value.VERY_GOOD))
    agent_pref.add_criterion_value(CriterionValue(hydrogen_engine, CriterionName.NOISE,
                                                  Value.VERY_GOOD))
